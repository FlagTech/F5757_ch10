...
- 如果存在 PROGRESS.md → 請讀取它：
  ...
  - 對於 [/] 的項目 → 問我是否要「重新規劃」這一項，還是「繼續上次的進度」。  
    ...
    - 若選擇繼續 → 沿用原有內容，維持原規劃。  
- 如果不存在 PROGRESS.md → 請根據 $ARGUMENTS 的任務內容，自動生成新的 PROGRESS.md 草稿，內容需包含：  
  - 任務清單（[ ] 狀態）  
  - 每項任務的檔案範圍  
  - 每項任務的摘要（簡述要修改什麼）  
  - 狀態更新規則（[ ] → 未開始；[/] → 進行中；[x] → 完成）

在規劃時，請確保任務數量控制在 3–8 項，並附上驗收條件（可手測或極薄測試）
...
6) 更新 @PROGRESS.md：  
   - 將該項目狀態改為 [x]（已完成）。  
   - 在該項下方新增一行，記錄「Commit 編號（前 7 碼）」與「提交訊息摘要」。
7) 若所有項目均為 [x] → 表示任務全部完成。
   - 將 PROGRESS.md 重新命名為 `YYYYMMDD_revisedContent.md`（以當天日期命名）
   - 若有 `log_history` 資料夾 → 儲存其中並刪除原檔。
   - 若無 → 自動建立 `log_history` 資料夾後再儲存。 
   - 最後依 @.claude/commands/pack-zh.md 流程提交變更。
